...
export default function Result() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/result/${code}`)
      ...
      .catch(() => {
        // API 실패 시 로컬 store fallback
        if (storeInput && storeChart) {
          setInput(storeInput)
          setReading(generateReading(storeChart, storeInput.nickname))
          setPaid(false)
          setLoading(false)
        } else {
          navigate('/input')
  ...
  const handleShare = useCallback(async () => {
    ...
    const title = input ? `${input.nickname}님의 별지도` : '별지도'
    if (navigator.share) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-text-muted font-serif italic">
        별을 읽고 있습니다...
      </div>
    )
  }
  ...
  // ── 결제 완료 시 데이터 준비 ──
  const basicSections = reading
    ? reading.sections.filter(s => BASIC_IDS.includes(s.id))
    : []
  const otherSections = reading
    ? SECTION_ORDER.map(id => reading.sections.find(s => s.id === id)).filter((s): s is ReadingSection => !!s)
    : []
  ...
  // ── 밤하늘 (chart 데이터 있을 때만) ──
  const nightSkyEl = storeChart ? (
    <>
      <section className="px-8 mb-6">
        <NightSky chart={storeChart} />
      </section>
  ...
  // ── 공유 + 새 별지도 버튼 ──
  const footerButtons = (
    <div className="px-8 text-center mt-12">
      <button
        onClick={handleShare}
